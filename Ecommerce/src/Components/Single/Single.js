  ...
  const handleclick = async (data, count, user) => {
    try {
      const response = await axios.post(
        "http://localhost:8080/cart/cartproduct",
        {
          user_id: user,
          product_id: data.id,
        ...
        <div className="product-details">
          ...
          <div className="buttons">
            <div className="button-container">
              <button className="decrement-btn" onClick={dclick}>
                -
              </button>
              <span className="quantity">{count}</span>
              <button className="increment-btn" onClick={click}>
                +
              </button>
            </div>
            {isauthenticated ? (
              <Link
                onClick={() => handleclick(data, count, user)}
            ...
            <Link className="back" style={{ textDecoration: "none" }} to={"/"}>
              Back
            </Link>
          </div>
        </div>
